...
describe('AppController (e2e)', () => {
  ...
  describe('Create all entities', () => {
    ...
    it('should create a guard relation between user1 and user2', async () => {
      await app.post('/guard-relation').send({
        "guarded_id": user1Id,
        "guardian_id": user2Id
      })
        .expect(201)
    ...
    it('should asign user1 to a guarded role', async () => {
      await app.get("/role")
        ...
        .expect((res) => {
          ...
          guardedRole = res.body.find(role => role.role_type === "GUARDED").id
        })
      await app.post("/user-role-relation").send({
        "user_id": user1Id,
        "role_id": guardedRole
    ...
    it('should asign user2 to a guardian role', async () => {
      await app.get("/role")
        ...
        .expect((res) => {
          expect(res.body[0]).toHaveProperty('id');
          guardianRole = res.body.find(role => role.role_type === "GUARDIAN").id
        })
      await app.post("/user-role-relation").send({
        "user_id": user2Id,
    ...
    it('should create an early exit', async () => {
      await app.post('/early-exit').send({
  ...
  describe('Delete all entities', () => {
    ...
    it('should delete the early exit', async () => {
      await app.delete(`/early-exit/${earlyExitId}`).expect(200).expect((res) => {
        expect(res.body).toHaveProperty('id');
      })
    });

    it('should delete the user2-role-relation', async () => {
      await app.delete(`/user-role-relation?role=${guardianRole}&user=${user2Id}`)
    });

    it('should delete the user1-role-relation', async () => {
      await app.delete(`/user-role-relation?role=${guardedRole}&user=${user1Id}`)
    });

    it('should delete the guard relation between user1 and user2', async () => {
      await app.delete(`/guard-relation?guardian=${user2Id}&guarded=${user1Id}`)
    });

    it('should delete a user', async () => {
      await app.delete(`/user/${user1Id}`)
        .expect(200)
        .expect((res) => {
          expect(res.body).toHaveProperty('id');
        })
